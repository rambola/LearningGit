    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
                ...
                boolean isDeviceNonTouchand12Key = NonTouchUtils.isDeviceNonTouchAnd12Key(context);
                boolean isAttDevice = context.getResources().getBoolean(
                        com.android.internal.R.bool.config_product_att);

                if (isAttDevice && isDeviceNonTouchand12Key) {
                    //For Such a device, Low battery notification/dialog should be shown in all cases
                    isPowerSaver = false;
                }
                if (!plugged
                        && !isPowerSaver
