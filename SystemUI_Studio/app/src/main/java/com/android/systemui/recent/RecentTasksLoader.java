...
public class RecentTasksLoader implements View.OnTouchListener {
        ...
        mTaskLoader = new AsyncTask<Void, ArrayList<TaskDescription>, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ...
                for (int i = first, index = 0; i < numTasks && (index < MAX_TASKS); ++i) {
                    ...
                    // Don't load the running, exclude from history activity only sonim PTT.
                    // XP5_VZW Bug 65718. start 11-04-2016
                    if (intent != null && intent.getComponent() != null
                            && intent.getComponent().getPackageName() != null
                            && intent.getComponent().getPackageName()
                                    .equals(getPttPlusPackageName(mContext))
                            && (intent.getFlags() & Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS) != 0) {
                        continue;
                    }
                    // XP5_VZW Bug 65718. end 11-04-2016
    ...
    private void loadThumbnailsAndIconsInBackground(
        ...
        try {
            ...
            Log.e(TAG, "Exception when loading recents " + e);
        }

    }

    private String getPttPlusPackageNameFromPkgMngr(Context context){
        final PackageManager pm = context.getPackageManager();
        Log.d(TAG, "getPttPlusPackageNameFromPkgMngr()");	//get a list of installed apps.
        List<ApplicationInfo> installedApps = pm.getInstalledApplications(PackageManager.GET_META_DATA);
        for (ApplicationInfo packageInfo : installedApps) {
            if((packageInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {
                //system app
                Bundle b = packageInfo.metaData;
                if (b!= null && b.containsKey("app_key_ptt_priority")) {
                    Log.d(TAG, "meta data key PRESENT: packageInfo.packageName = " + packageInfo.packageName);
    ...
    private String getPttPlusPackageName(Context context){
        String pkgName = Secure.getString(context.getContentResolver(),"ptt_app_package" );
        if(pkgName == null || pkgName.equals("")){
            pkgName = getPttPlusPackageNameFromPkgMngr(context);
            if( Secure.getString(context.getContentResolver(),"ptt_app_package") == null){
                Secure.putString(context.getContentResolver(), "ptt_app_package", pkgName);
            }
        }
        return pkgName;
    }
