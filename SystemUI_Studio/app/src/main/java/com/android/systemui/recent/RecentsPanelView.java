...
public class RecentsPanelView extends FrameLayout implements OnItemClickListener, RecentsCallback,
    ...
    public void handleSwipe(View view) {
        ...
        try {
            if (rpsService != null && rpsService.IsRPSEnabled()) {
                boolean preventForceStop = false;

                preventForceStop = rpsService.preventForceStop(ad.packageName);

                if (preventForceStop) {

                    if (DEBUG)
                        Log.d(TAG, "**** SONIM APP EXIT");
                    // User cann't force stop RPS privileged application
                    return;
                }
            }
        } catch (RemoteException e) {
            Log.i(TAG, "handleSwipe" + e);
        }
        // ~Sonim RPS
    ...
    private void showEmptyAndGoBack() {
    ...
    public void handleLongPress(
        ...
        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                ...
                } else if(item.getItemId() == R.id.recent_remove_all) {
                    removeAllViews();
                } else {
                    return false;
                }
    ...
    public void removeAllViews() {
        if (mRecentsContainer != null) {
            ...
            if (mRecentTaskDescriptions != null && mRecentTaskDescriptions.size() > 0) { // nullpointer exception
                int sizeBeforeRemoval = mRecentTaskDescriptions.size();
                for (int i = 0; i <= sizeBeforeRemoval; i++) {
                    View view = container.getChildAt(mRecentTaskDescriptions.size() - 1);
