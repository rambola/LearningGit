...
public class NotificationData {
    ...
    private final String SONIM_LAUNCHER_PACKAGE_NAME = "com.sonim.borqs.launcher";
    ...
    public ArrayList<Entry> getActiveNotifications() {
        Log.e("NotificationData", "NotificationData, getActiveNotifications(), mSortedAndFiltered.size():  "+mSortedAndFiltered.size());
    ...
    public void updateRankingAndSort(RankingMap ranking) {
        if (ranking != null) {
            mRankingMap = ranking;
            synchronized (mEntries) {
        ...
        synchronized (mEntries) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if(NotificationFilterListener.mPackageList != null) {
                    Log.i("SystemUI", "filterAndSort(), sbn.getPackageName(): " + sbn.getPackageName());
                    for (int j = 0; j < NotificationFilterListener.mPackageList.size(); j++) {
                        Log.i("SystemUI", "filterAndSort(), NotificationFilterListener.mPackageList.get(j): "+ NotificationFilterListener.mPackageList.get(j));
                    }
                }

                if(NotificationFilterListener.mPackageList != null) {
                    Log.i("NotificationData","filterAndSort(), came to first if");
                    Log.i("NotificationData","filterAndSort(), came to first if, SystemPropreties value: "+SystemProperties.get("persist.sys.mcl_mode","false"));
                    if(/*("true").equalsIgnoreCase(SystemProperties.get("persist.sys.mcl_mode","false")) &&*/ (!(NotificationFilterListener.mPackageList.contains(sbn.getPackageName()) || sbn.getPackageName().contentEquals(ANDROID_NAME) || sbn.getPackageName().contentEquals(SONIM_LAUNCHER_PACKAGE_NAME)))) {
//                    if(("true").equalsIgnoreCase(SystemProperties.get("persist.sys.mcl_mode","false")) && (!(NotificationFilterListener.mPackageList.contains(sbn.getPackageName())/* || sbn.getPackageName().contentEquals("android") || sbn.getPackageName().contentEquals("com.sonim.borqs.launcher")*/))) {
                        Log.i("NotificationData","filterAndSort(), came to second if, not adding the package: "+sbn.getPackageName());
                        continue;
                    }
                }

                Log.i("NotificationData","Notification being added for package : "+sbn.getPackageName());
