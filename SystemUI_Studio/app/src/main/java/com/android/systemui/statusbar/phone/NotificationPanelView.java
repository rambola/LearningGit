...
public class NotificationPanelView extends PanelView implements
    ...
    private void setQsExpanded(boolean expanded) {
        ...
        /*if (changed && ("false").equalsIgnoreCase(SystemProperties.get("persist.sys.mcl_mode","false"))) {*/
        if (changed && NotificationFilterListener.mPackageList.size() > 0) {
...
//        if (("true").equalsIgnoreCase(SystemProperties.get("persist.sys.mcl_mode","false")) && height != 0.0f) {
        if(NotificationFilterListener.mPackageList.size() > 0 && height != 0.0f){
            return;
        } else {
    ...
    public void expand(boolean animate) {
        // XP: Non-touch support
        // Disable notification panel expansion
        if (!NonTouchUtils.isDeviceNonTouchAnd12Key(getContext())) {
