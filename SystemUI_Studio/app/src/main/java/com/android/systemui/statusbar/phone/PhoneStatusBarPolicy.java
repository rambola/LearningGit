...
//Sonim RPS
import android.app.SentrifyManager;
import android.os.IRPSService;
...
public class PhoneStatusBarPolicy implements Callback, RotationLockController.RotationLockControllerCallback, DataSaverController.Listener {
    ...
    // Sonim RPS
    private boolean mIsRpsFeatureEnabled = false;
    // ~Sonim RPS
    ...
    public PhoneStatusBarPolicy(Context context, StatusBarIconController iconController,
        ...
        // Sonim RPS
        mIsRpsFeatureEnabled = context.getResources().getBoolean(
                com.android.internal.R.bool.feature_sonim_rps);
        if (mIsRpsFeatureEnabled) {
            filter.addAction(Intent.ACTION_RPS_STATE_CHANGED);
            filter.addAction(Intent.ACTION_SHOW_NOTIFICATION_FROM_ERS);
        }
        // ~Sonim RPS
        ...
        // Sonim RPS
        if (mIsRpsFeatureEnabled) {
            IRPSService rpsService = ((SentrifyManager)mContext.getSystemService(Context.SENTRIFY_SERVICE)).getSentrifyService();
            if (rpsService == null) return;

            mIconController.setIcon("rps_state", R.drawable.stat_sys_rps_active, null);
            try {
                mIconController.setIconVisibility("rps_state", rpsService.IsRPSEnabled());
            } catch (RemoteException e) {
    ...
    // Sonim RPS
    private final void updateRPSState(Intent intent) {
        int state_value = intent.getIntExtra("state_value", 0);
        int [] icons = { R.drawable.stat_sys_rps_on,
                R.drawable.stat_sys_rps_on,  R.drawable.stat_sys_rps_active};

        Log.d(TAG, "updateRPSState :" + String.format("%d", state_value));
        mIconController.setIcon("rps_state", icons[state_value], null);
        mIconController.setIconVisibility("rps_state", (state_value > 0));

    }

    private final void showERSNotification(Intent intent) {
        final int NOTIFY_ME_ID = 1727;
        Resources res = mContext.getResources();
        String tickerTxt = res.getString(R.string.sonim_rps_ticker_text);
        String packageName = intent.getStringExtra("ers_notification");
        ...
        final NotificationManager mgr = (NotificationManager)mContext
                .getSystemService(mContext.NOTIFICATION_SERVICE);
        Notification note = new Notification(android.R.drawable.ic_menu_info_details, tickerTxt,
                System.currentTimeMillis());
        note.flags |= Notification.FLAG_AUTO_CANCEL;
        PendingIntent contentIntent = PendingIntent.getActivity(mContext, 0, new Intent(), 0);
        note.setLatestEventInfo(mContext, res.getString(
                R.string.sonim_rps_notify_title,appName),
                res.getString(R.string.sonim_rps_notify_content),
                contentIntent);
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // Sonim RPS
            else if (mIsRpsFeatureEnabled
                    && action.equals(Intent.ACTION_RPS_STATE_CHANGED)) {
                updateRPSState(intent);
            } else if (mIsRpsFeatureEnabled
                    && action.equals(Intent.ACTION_SHOW_NOTIFICATION_FROM_ERS)) {
                showERSNotification(intent);
            }
            // ~Sonim RPS
        }
    };
