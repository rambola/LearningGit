...
class TelephonyIcons {
    ...
    static final int TELEPHONY_NUM_LEVELS = 6;

    //GSM/UMTS
    static final int TELEPHONY_NO_NETWORK = R.drawable.ic_stat_sys_signal_fully_no_att;
    ...
    static final int[][] TELEPHONY_SIGNAL_STRENGTH = {
        { R.drawable.stat_sys_signal_0,
          ...
          R.drawable.stat_sys_signal_4},
        { R.drawable.ic_stat_sys_signal_fully_0_att,
          R.drawable.ic_stat_sys_signal_fully_1_att,
          R.drawable.ic_stat_sys_signal_fully_2_att,
          R.drawable.ic_stat_sys_signal_fully_3_att,
          R.drawable.ic_stat_sys_signal_fully_4_att,
          R.drawable.ic_stat_sys_signal_fully_5_att }
    };
    ...
    private static boolean isATTorSL = false;
    ...
    static void readIconsFromXml(Context context) {
        ...
        isATTorSL = Resources.getSystem().getBoolean(com.android.internal.R.bool.config_product_att) ||
                Resources.getSystem().getBoolean(com.android.internal.R.bool.config_product_southernl);
        isInitiated = true;
    }
    ...
    static void updateDataType(int slot, int type, boolean showAtLeast3G,
        ...
        switch (type) {
            ...
            case TelephonyManager.NETWORK_TYPE_HSPA:
                if (isATTorSL) {
                    setRATas4G(slot);
                } else if (hspaDistinguishable || MobileSignalController.isCarrierOneSupported()) {
                    mSelectedDataTypeDesc[slot] = mDataTypeDescriptionArray[type];
            ...
            case TelephonyManager.NETWORK_TYPE_HSPAP:
                if (isATTorSL) {
                    setRATas4G(slot);
                } else if (hspaDistinguishable || MobileSignalController.
            ...
            case TelephonyManager.NETWORK_TYPE_LTE_CA:
                // Impl of <CDR-ONS-255> Show 4G LTE icon for LTE network
                if (isATTorSL) {
                    mSelectedDataActivityIndex[slot] = DATA_TYPE_LTE;
                    mSelectedDataTypeIcon[slot] = ICON_4G_LTE;
                    mSelectedQSDataTypeIcon[slot] = QS_DATA_4G_LTE;
                    mSelectedDataTypeDesc[slot] = mDataTypeGenerationDescArray[1];
                    mSelectedSignalStreagthIndex[slot] = SIGNAL_STRENGTH_TYPE_4G;
                } else if (!show4GforLte || MobileSignalController.isCarrierOneSupported()) {
                    mSelectedDataActivityIndex[slot] = DATA_TYPE_LTE;
                    mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                            dataTypeArray[type], null, NS);
    ...
    /*
     * Impl of <CDR-ONS-255> Show 4G icons for UMTS, HSDPA, HSUPA, HSPA and HSPAP
     */
    private static void setRATas4G(int slot) {
        mSelectedDataActivityIndex[slot] = DATA_TYPE_4G;
        mSelectedDataTypeIcon[slot] = mRes.getIdentifier(
                mDataTypeGenerationArray[1], null, NS);
        mSelectedQSDataTypeIcon[slot] = QS_DATA_4G;
